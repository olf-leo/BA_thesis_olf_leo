...
def calculate_sim_force(pressure_range, workpiece_name, distance, flow_mode):
    sim_forces = []
    conv = 100000
    if (flow_mode):
        conv = 1
    ...
    for i in range(0, len(pressure_range)):
        results = calc_force(workpiece_name, [0,0,0], 1.17, distance, 4, pressure_range[i]*conv, 300, flow_mode, False, False, False)
        sim_forces.append(results[0]*(results[3]**2))
    
    return sim_forces

def calculate_sim_force_batch(pressure_range, workpiece_names, distance, flow_mode):
    sim_forces = []
    for i in range(0, len(workpiece_names)):
        sim_forces.append(calculate_sim_force(pressure_range, workpiece_names[i], distance, flow_mode))
...
def force_size_sweep(workpiece_names, pressure, distance):
    forces = []
    for i in range(0, len(workpiece_names)):
        results = calc_force(workpiece_names[i], [0,0,0], 1.17, distance, 4, pressure*100000, 300, False, False, False, False)
...
flow_data_raw = extract_flow_data(flow_data_filepath)
flow_avgs, flow_cis = calculate_binned_stats(flow_data_raw)
sim_force_flow = calculate_sim_force(flow_avgs, '4dx1h_disc', distance, True)
...
#             0    1    2     3     4     5     6     7
diameters = [7.5, 10.0, 15.0, 20.0, 25.0, 30.0, 40.0] #5.0 removed
diameter_index = 6
workpiece_names = [ 
                   '0_75dx1h_disc',
                   '1dx1h_disc',
...
#testdata_filepath = '/Users/leonardolfens/Desktop/Python_Match/testdata/06_03_26-17_14_06.03.2026fulltestrun.csv'
...
sim_forces = calculate_sim_force(pressures, workpiece_names[diameter_index], distance, False)
#forces_size_sweep = force_size_sweep(workpiece_names, pressures[pressure_index], distance)

forces_size_sweep = []
for i in range(0, len(workpiece_names)):
    results = calc_force(workpiece_names[i], [0,0,0], 1.17, distance, 4, pressures[pressure_index]*100000, 300, False, False, False, False)
